...
public class QuickLocationBar extends View {
    ...
    private void init() {
        ...
        circlePaint.setColor(getResources().getColor(R.color.com_facebook_blue));
...
//}
                ...
                if (selectChars.equals(characters.get(i))) {
                    canvas.drawCircle(xPos + paint.measureText(characters.get(i)) / 2, yPos - singleHeight / 2 - 8, width / 3, circlePaint);
                    paint.setColor(Color.WHITE);
                } else {
                    paint.setColor(getResources().getColor(R.color.black));
                }
                // 在画布上绘制字符
                canvas.drawText(characters.get(i), xPos, yPos - singleHeight / 2, paint);
